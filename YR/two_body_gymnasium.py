...
class SpacecraftEnv(Env):
    ...
    def step(self, a_ind):
        ...
        if action[0] == 1:  self.orbit_propogation(0.5 * period)  # Wait 1.5 * T
    ...
    def add_trajectory(self, state=False, color="red"):
        # Plot one full orbit
        state = self.state if state is False else state
        a = self.get_semimajor_axis(state)
        period = 2 * np.pi * np.sqrt(a ** 3 / self.earth_mu)
        trajectory = self.orbit_propogation(period, state)
        ...
        self.fig.add_trace(go.Scatter3d(x=x, y=y, z=z, mode='lines', line=dict(color=color)))
    ...
    def render(self, mode='human'):
        ...
        self.add_trajectory(self.initial_orbit, color="blue")
        self.add_trajectory(self.final_orbit, color="red")
        self.add_trajectory(self.state, color="purple")
        self.fig.update_xaxes(range=[- 2,  2])
        self.fig.update_yaxes(range=[-(self.earth_radius + 35786.0)/ 2, (self.earth_radius + 35786.0)/ 2])
        self.fig.show()
